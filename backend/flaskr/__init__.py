    ...
    try:
      ...
      if quiz_category_id > 0:
        if len(previous_questions) > 0:
          questions = Question.query.filter(Question.category == quiz_category_id).filter(~Question.id.in_(previous_questions)).order_by(func.random()).all()
        else:
          questions = Question.query.filter(Question.category == quiz_category_id).order_by(func.random()).all()
      else:
        if len(previous_questions) > 0:
